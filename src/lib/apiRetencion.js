...
function construirQuery(filtros = {}) {
  ...
  if (!esFiltroVacio(filtros.subtipo)) params.set("subtipo", filtros.subtipo);
  if (!esFiltroVacio(filtros.search)) params.set("search", filtros.search);

  params.set("ordering", filtros.ordering || "-fecha_os");
...
export function obtenerOpcionesRetencion(options = {}) {
  return http("/retencion/api/ordenes-servicio/opciones/", options);
}
...
export function obtenerOrdenesRetencion(filtros = {}, options = {}) {
  ...
  return http(
    `/retencion/api/ordenes-servicio/ligero/${query ? `?${query}` : ""}`,
    options,
  );
}
...
export const apiRetencion = {
  list: (filtros = {}, options = {}) => {
    ...
    return http(
      `/retencion/api/ordenes-servicio/${query ? `?${query}` : ""}`,
      options,
    );
  },

  ligero: (filtros = {}, options = {}) => {
    return obtenerOrdenesRetencion(filtros, options);
  },

  opciones: (options = {}) => {
    return obtenerOpcionesRetencion(options);
  },

  get: (id) => {
    return http(`/retencion/api/ordenes-servicio/${encodeURIComponent(id)}/`);
  },
};
