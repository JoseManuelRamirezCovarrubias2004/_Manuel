...
function MiniToggle({ value, onChange }) {
  return (
    <button
      ...
      role="switch"
      aria-checked={value}
      className={`relative inline-flex items-center w-10 h-5 rounded-full transition-colors duration-300 focus:outline-none flex-shrink-0 ${
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className={`relative inline-flex items-center w-14 h-7 rounded-full transition-colors duration-300 focus:outline-none ${
                  swiftActivo ? "bg-[#131E5C]" : "bg-gray-300"
                }`}
              ...
              >
                <span
                  className={`inline-block w-5 h-5 bg-white rounded-full shadow-md transform transition-transform duration-300 ${
                    swiftActivo ? "translate-x-8" : "translate-x-1"
                  }`}
                />
              </button>
              <span
          ...
          <div className="flex flex-col gap-1 flex-1 min-w-[260px]">
            ...
            <div
              className={`overflow-hidden transition-all duration-500 ease-in-out ${
                mostrarHorarios ? "max-h-[600px] opacity-100 mt-3" : "max-h-0 opacity-0"
              }`}
            >
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                ...
                {DIAS.map((dia, idx) => {
                  ...
                  return (
                    <div
                      key={dia.id}
                      className={`grid grid-cols-[90px_1fr_1fr_1fr_80px] items-center px-4 py-2.5 gap-2 text-sm transition-colors duration-200 ${
          ...
          <div key={seccion.id} className="bg-white rounded-xl shadow px-8 py-5">
            ...
            <div className="relative">
              ...
              <textarea
                ...
                rows={3}
                className={`w-full border rounded-lg px-4 py-3 text-sm text-gray-600 resize-y transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-300 ${
                  editando[seccion.id]
