...
export default function CampanasMeta() {
  ...
  const [compA, setCompA] = useState("2026-5");
  const [compB, setCompB] = useState("2025-5");
...
, [DATOS_RAW]);
  ...
  const datosFiltrados = useMemo(()=>{
    return DATOS_RAW.filter(c=>{
      if(año!=="Todos" && c.año!==parseInt(año)) return false;
      if(mes!=="Todos" && c.mes!==parseInt(mes)) return false;
      if(semana!=="Todas" && c.semana!==parseInt(semana)) return false;
      if(sucursal!=="Todas" && c.sucursal!==sucursal) return false;
      return true;
    });
  },[año,mes,semana,sucursal,DATOS_RAW]);

  const filtrarComp = (key) => {
    if(tipoComp==="años"){
      return DATOS_RAW.filter(c=>{
        if(c.año!==parseInt(key)) return false;
        if(sucursal!=="Todas" && c.sucursal!==sucursal) return false;
        return true;
      });
    } else {
      const [y,m] = key.split("-");
      return DATOS_RAW.filter(c=>{
        if(c.año!==parseInt(y)||c.mes!==parseInt(m)) return false;
        if(sucursal!=="Todas" && c.sucursal!==sucursal) return false;
        return true;
      });
    }
  ...
  const datosA = useMemo(()=>filtrarComp(compA),[compA,tipoComp,sucursal,DATOS_RAW]);
  const datosB = useMemo(()=>filtrarComp(compB),[compB,tipoComp,sucursal,DATOS_RAW]);

  const datosActivos = modoComp ? datosA : datosFiltrados;
  const datosComp = modoComp ? datosB : [];

  const labelA = tipoComp==="años" ? compA : (()=>{ const [y,m]=compA.split("-"); return `${MESES[parseInt(m)-1].slice(0,3)} ${y}`; })();
  const labelB = tipoComp==="años" ? compB : (()=>{ const [y,m]=compB.split("-"); return `${MESES[parseInt(m)-1].slice(0,3)} ${y}`; })();
  ...
  const opcionesAñoMes = useMemo(()=>{
    const set=new Set();
    DATOS_RAW.forEach(c=>set.add(`${c.año}-${c.mes}`));
    return Array.from(set).sort().map(v=>{
      const [y,m]=v.split("-");
      return {value:v, label:`${MESES[parseInt(m)-1].slice(0,3)} ${y}`};
    });
  },[DATOS_RAW]);

  const opcionesAño = useMemo(()=>
    añosDisponibles.map(a=>({value:String(a),label:String(a)}))
  ,[añosDisponibles]);

  const mesesDisponibles = useMemo(()=>{
    const src = año==="Todos" ? DATOS_RAW : DATOS_RAW.filter(d=>d.año===parseInt(año));
          ...
          {["tabla","graficas"].map(v=>(
            <button key={v} onClick={()=>setVista(v)}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium border transition ${
                vista===v ? "text-white border-transparent" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50"
              }`}
      ...
      <div className="rounded-xl border border-gray-200 overflow-hidden mb-5">
        ...
        <div className="flex items-center gap-1 px-4 py-3 border-b border-gray-200 bg-gray-50/60 flex-wrap">
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-full text-sm font-medium border transition ml-1 ${
              año==="Todos"
                ? "text-white border-transparent"
        ...
        <div className="flex flex-wrap items-end gap-4 px-4 py-3 bg-white">
          ...
          <div>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium border transition h-[38px] ${
                modoComp ? "text-white border-transparent" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50"
              }`}
          ...
          <div className="ml-auto flex items-end gap-2 flex-wrap">
            <button
              onClick={()=>{ setAño(String(new Date().getFullYear())); setMes(String(new Date().getMonth()+1)); }}
            ...
            <button
              onClick={()=>{ setAño(String(new Date().getFullYear())); setMes("Todos"); }}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium text-white transition"
