...
function TeamsModal({ open, onClose, onCreated }) {
    ...
    const fetchTeams = useCallback(async () => {
        ...
        try {
            ...
            if (data && data.length > 0 && !selectedTeamForInvite) {
                setSelectedTeamForInvite(data[0]);
            }
        } catch (e) { console.error(e); }
        finally { setLoading(false); }
    }, []);

    const loadTeamMembersAndInvites = useCallback(async (teamId) => {
        if (!teamId) return;
        setLoadingInvites(prev => ({ ...prev, [teamId]: true }));
        try {
            const [members, invites] = await Promise.all([
                apiClickup.listMembers(teamId),
                apiClickup.listInvites(teamId)
            ]);
            setMembersByTeam(prev => ({ ...prev, [teamId]: Array.isArray(members) ? members : [] }));
            setInvitesByTeam(prev => ({ ...prev, [teamId]: Array.isArray(invites) ? invites : [] }));
        } catch (e) {
            console.error(e);
            setMembersByTeam(prev => ({ ...prev, [teamId]: [] }));
            setInvitesByTeam(prev => ({ ...prev, [teamId]: [] }));
        }
        finally {
            setLoadingInvites(prev => ({ ...prev, [teamId]: false }));
        }
    }, []);
    ...
    useEffect(() => {
        if (selectedTeamForInvite && selectedTeamForInvite.id) {
            loadTeamMembersAndInvites(selectedTeamForInvite.id);
        }
    }, [selectedTeamForInvite]);

    const searchUsersForInvite = useCallback(async (term) => {
        ...
        try {
            const results = await apiClickup.searchUsers(trimmed);
            setInviteSearchResults(Array.isArray(results) ? results : []);
        } catch (e) {
            console.error(e);
            setInviteSearchResults([]);
        }
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            if (inviteSearchTerm) searchUsersForInvite(inviteSearchTerm);
    ...
    async function sendInvite() {
        if (!selectedTeamForInvite || !selectedUserToInvite) {
            alert("Selecciona un usuario primero");
            return;
        }
        setSendingInvite(true);
        try {
            await apiClickup.invite(selectedTeamForInvite.id, {
                usuario_id: Number(selectedUserToInvite.id),
                rol: inviteRole,
            });
            await loadTeamMembersAndInvites(selectedTeamForInvite.id);
            setSelectedUserToInvite(null);
            setInviteSearchTerm("");
            setInviteSearchResults([]);
            setInviteRole("MEMBER");
            alert(`Invitación enviada a ${selectedUserToInvite.name}`);
        } catch (e) {
    ...
    async function cancelInvite(inviteId) {
        if (!selectedTeamForInvite || !confirm("¿Cancelar esta invitación?")) return;
        try {
            await apiClickup.cancelInvite(selectedTeamForInvite.id, inviteId);
            await loadTeamMembersAndInvites(selectedTeamForInvite.id);
        } catch (e) {
            alert(e.message || "Error al cancelar invitación");
        }
    }

    async function removeMember(memberId, memberName) {
        if (!selectedTeamForInvite || !confirm(`¿Eliminar a ${memberName} del equipo?`)) return;
        try {
            await apiClickup.removeMember(selectedTeamForInvite.id, memberId);
            await loadTeamMembersAndInvites(selectedTeamForInvite.id);
        } catch (e) {
            alert(e.message || "Error al eliminar miembro");
        }
    }
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-4xl overflow-hidden rounded-2xl border border-black/10 bg-white shadow-2xl">
                ...
                <div className="flex flex-col md:flex-row max-h-[70vh] overflow-hidden">
                    ...
                    <div className="w-full md:w-2/5 border-r border-black/10 p-5 overflow-y-auto">
                    ...
                    <div className="w-full md:w-3/5 p-5 overflow-y-auto">
                        {!selectedTeamForInvite ? (
                            <div className="flex flex-col items-center justify-center h-full min-h-[300px] text-center">
                                ...
                                <div className="text-sm text-black/40">Selecciona un equipo para gestionar sus miembros</div>
                            </div>
                        ) : (
                            <div>
                                <div className="mb-4">
                                    <div className="text-xs font-extrabold uppercase tracking-widest text-black/35">Miembros</div>
                                    <div className="text-sm font-black text-[#131E5C]">{selectedTeamForInvite.name}</div>
                                </div>
                                ...
                                <div className="mb-4 p-3 rounded-xl bg-slate-50">
                                    ...
                                    {inviteSearchResults.length > 0 && !inviteSearching && (
                                        <div className="mb-2 border rounded-xl overflow-hidden max-h-48 overflow-y-auto">
                                    ...
                                    {selectedUserToInvite && (
                                        <div className="mb-2 p-2 rounded-lg bg-emerald-50 border border-emerald-200 flex items-center justify-between">
                                            <div>
                                                <div className="text-sm font-bold text-emerald-800">{selectedUserToInvite.name}</div>
                                                <div className="text-xs text-emerald-600">{selectedUserToInvite.email}</div>
                                            </div>
                                            <button
                                                onClick={() => { setSelectedUserToInvite(null); setInviteSearchTerm(""); }}
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button
                                            onClick={sendInvite}
                                            disabled={!selectedUserToInvite || sendingInvite}
                                ...
                                <div className="mb-4">
                                    <div className="text-xs font-extrabold text-black/60 mb-2 flex items-center gap-2">
                                        ...
                                        Miembros del equipo ({membersByTeam[selectedTeamForInvite.id]?.length || 0})
                                    </div>
                                    {loadingInvites[selectedTeamForInvite.id] ? (
                                        <div className="text-center py-4"><Loader2 className="h-5 w-5 animate-spin text-black/30 mx-auto" /></div>
                                    ) : membersByTeam[selectedTeamForInvite.id]?.length === 0 ? (
                                        <div className="rounded-xl border border-dashed border-black/10 p-4 text-center text-xs text-black/40">Sin miembros en este equipo</div>
                                    ) : (
                                        <div className="space-y-2 max-h-[200px] overflow-y-auto">
                                            {membersByTeam[selectedTeamForInvite.id]?.map(member => (
                                                <div key={member.id} className="flex items-center justify-between rounded-xl border border-black/10 bg-white px-3 py-2">
                                                    <div>
                                                        <div className="text-sm font-bold text-[#131E5C]">{member.name}</div>
                                                        <div className="text-xs text-black/50">{member.email}</div>
                                                        <div className="text-[10px] text-black/40">Rol: {member.role}</div>
                                                    </div>
                                                    {member.role !== "OWNER" && (
                                                        <button
                                ...
                                <div>
                                    <div className="text-xs font-extrabold text-black/60 mb-2 flex items-center gap-2">
                                        ...
                                        Invitaciones pendientes ({invitesByTeam[selectedTeamForInvite.id]?.length || 0})
                                    </div>
                                    {loadingInvites[selectedTeamForInvite.id] ? (
                                        <div className="text-center py-4"><Loader2 className="h-5 w-5 animate-spin text-black/30 mx-auto" /></div>
                                    ) : invitesByTeam[selectedTeamForInvite.id]?.length === 0 ? (
                                        ...
                                        <div className="space-y-2 max-h-[150px] overflow-y-auto">
                                            {invitesByTeam[selectedTeamForInvite.id]?.map(inv => (
                                                <div key={inv.id} className="flex items-center justify-between rounded-xl border border-amber-200 bg-amber-50 px-3 py-2">
                                                    <div>
                                                        <div className="text-sm font-bold text-amber-800">{inv.invited_user?.name || inv.email || "Usuario"}</div>
                                                        <div className="text-xs text-amber-600">Rol: {inv.role}</div>
                                                        <div className="text-[10px] text-amber-500">Expira: {new Date(inv.expires_at).toLocaleDateString()}</div>
                                                    </div>
                                                    <button
                                                        onClick={() => cancelInvite(inv.id)}
                                                        className="rounded-lg border border-amber-300 bg-amber-100 p-1.5 text-amber-700 hover:bg-amber-200"
                                                    >
                                                        <X className="h-3.5 w-3.5" />
                                                    </button>
                                                </div>
                ...
                <div className="border-t border-black/10 p-4 flex justify-end">
                    <button onClick={onClose} className="rounded-xl border border-black/10 bg-white px-4 py-2 text-sm font-extrabold text-black/70 hover:bg-slate-50">
                        Cerrar
                    </button>
                </div>
            </div>
...
// ========== TASK MODAL CON ASIGNACIÓN DE USUARIOS ==========
function TaskModal({ open, onClose, task, lists, teamId, onSaved }) {
    ...
    const searchAssignees = useCallback(async () => {
        ...
        try {
            const results = await apiClickup.searchUsers(assigneeSearch);
            setAssigneeResults(Array.isArray(results) ? results : []);
        } catch (e) {
            console.error(e);
            setAssigneeResults([]);
        } finally {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            if (assigneeSearch) searchAssignees();
            else setAssigneeResults([]);
        }, 500);
        return () => clearTimeout(timer);
    ...
    async function handleSave() {
        ...
        try {
            ...
            if (task?.id) {
                ...
                if (nuevaTarea && nuevaTarea.id) currentTaskId = nuevaTarea.id;
            }
                    ...
                    <section>
                        ...
                        <div className="space-y-3">
                            <div>
                                ...
                                <input value={title} onChange={e => setTitle(e.target.value)} className={cls(inputBase, "mt-1")} placeholder="Nombre del plan de acción" />
                            </div>
                    ...
                    <section>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <textarea value={problema} onChange={e => setProblema(e.target.value)} className={cls(inputBase, "mt-1 min-h-[90px]")} placeholder="¿Cuál es el problema que se está atendiendo?" />
                            </div>
                            ...
                            <div className="rounded-xl border border-black/10 bg-slate-50 p-4">
                                ...
                                <div className="flex gap-2 mb-3">
                                    ...
                                    <button onClick={addSubtask} className="inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm font-extrabold text-white" style={{ backgroundColor: BRAND_BLUE }}>
                            ...
                            {/* Asignar usuarios */}
                            <div className="rounded-xl border border-black/10 bg-slate-50 p-4">
                                <div className="mb-3 text-xs font-extrabold text-[#131E5C] flex items-center gap-2">
                                    <UsersRound className="h-3.5 w-3.5" />
                                    Asignado a
                                </div>
                                    ...
                                    <div className="flex flex-wrap gap-2 mb-3">
                                        {assignedUsers.map(user => (
                                            <div key={user.id} className="flex items-center gap-2 rounded-full bg-[#131E5C]/10 px-3 py-1.5">
                                                ...
                                                <button onClick={() => setAssignedUsers(prev => prev.filter(u => u.id !== user.id))} className="text-black/40 hover:text-red-500">
                                                    <X className="h-3.5 w-3.5" />
                                                </button>
                                            </div>
                                        ))}
                                ...
                                <div className="relative">
                                    <input type="text" value={assigneeSearch} onChange={(e) => setAssigneeSearch(e.target.value)} className={cls(inputBase, "pr-8")} placeholder="Buscar usuario para asignar..." />
                                ...
                                {assigneeResults.length > 0 && (
                                    <div className="mt-1 border rounded-xl overflow-hidden max-h-48 overflow-y-auto">
                                        {assigneeResults.map(user => {
                                            const alreadyAssigned = assignedUsers.some(u => u.id === user.id);
                                            return (
                                                <button
                                                    key={user.id}
                                                    onClick={() => {
                                                        if (!alreadyAssigned) setAssignedUsers(prev => [...prev, user]);
                                                        setAssigneeSearch("");
                                                        setAssigneeResults([]);
                                                    }}
                                                    disabled={alreadyAssigned}
                                                    className={cls("w-full text-left px-3 py-2 hover:bg-slate-100 border-b last:border-b-0", alreadyAssigned && "opacity-50 bg-slate-50")}
                                                >
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        {alreadyAssigned && <span className="text-xs text-emerald-600 ml-auto">✓ Ya asignado</span>}
                                                    </div>
                                                </button>
                                            );
                                        })}
                                ...
                                {assignedUsers.length === 0 && (
                                    <p className="text-xs text-black/40 mt-2 text-center">No hay usuarios asignados. Busca y selecciona arriba.</p>
                                )}
                            </div>
                        </div>
                    </section>
                    ...
                    <section>
                        ...
                        <div className="rounded-xl border border-dashed border-black/20 bg-slate-50 p-4 text-center">
                            ...
                            <label className="cursor-pointer text-sm font-extrabold text-[#131E5C] hover:underline">
                                ...
                                <input type="file" multiple accept=".png,.jpg,.jpeg,.pdf,.mp4,.mov,.webm,.mp3,.wav,.m4a" className="hidden" onChange={e => setEvidencias(prev => [...prev, ...Array.from(e.target.files || [])])} />
                            </label>
                                    ...
                                    return (
                                        <div key={i} className="flex items-center justify-between gap-3 rounded-xl border border-black/10 bg-white px-3 py-2">
                                            <div className="flex items-center gap-2 min-w-0">
                                                <span className="text-lg">{isImg ? "🖼️" : isPdf ? "📄" : isVideo ? "🎬" : isAudio ? "🎵" : "📎"}</span>
                                                <span className="text-xs font-semibold text-black/70 truncate">{f.name}</span>
                                            </div>
                                            <button onClick={() => setEvidencias(prev => prev.filter((_, j) => j !== i))} className="shrink-0 rounded-lg border border-rose-200 bg-rose-50 p-1.5 text-rose-600 hover:bg-rose-100">
                                                <X className="h-3.5 w-3.5" />
                                            </button>
                                        </div>
                                    );
            ...
            <div className="p-4">
                <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex shrink-0 items-center gap-1">
                        <button onClick={() => onEdit(task)} className="inline-flex items-center justify-center rounded-xl border border-black/10 bg-white p-2 text-black/60 hover:bg-slate-50">
                            <Pencil className="h-3.5 w-3.5" />
                        </button>
                        <button onClick={() => onDelete(task)} className="inline-flex items-center justify-center rounded-xl border border-rose-200 bg-rose-50 p-2 text-rose-600 hover:bg-rose-100">
                            <Trash2 className="h-3.5 w-3.5" />
                        </button>
                    </div>
                </div>
                ...
                return (
                    <div key={col} className="flex flex-col rounded-2xl border border-black/10 bg-slate-50/80 overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-3 border-b border-black/[0.07]">
                            ...
                            {list && (
                                <button onClick={() => onCreateInCol(list.id)} className="inline-flex h-6 w-6 items-center justify-center rounded-lg border border-black/10 bg-white text-black/50 hover:bg-slate-100 hover:text-[#131E5C]">
                ...
                <table className="min-w-full text-left text-sm">
                        ...
                        <tr>
                            {[["title", "Título"], ["list_name", "Estado"], ["priority", "Prioridad"], ["start_date", "Fecha inicio"], ["due_date", "Fecha límite"], ["causa", "Causa"]].map(([k, l]) => (
                                <th key={k} className="px-4 py-3">
                                    <button onClick={() => toggleSort(k)} className="inline-flex items-center font-bold text-xs">
                                        {l}<SortIcon k={k} />
                                    </button>
                                </th>
                            ))}
                    ...
                    <tbody className="divide-y divide-black/[0.06]">
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan={9} className="px-4 py-10 text-center text-sm text-black/40">Sin planes con estos filtros.</td>
                            </tr>
                        ) : (
                            sorted.map(task => {
                                    ...
                                    <tr key={task.id} className="hover:bg-slate-50/60 cursor-pointer" onDoubleClick={() => onEdit(task)}>
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1">
                                                {task.assigned && task.assigned.slice(0, 2).map((a, i) => <UserAvatar key={i} user={a} size="sm" />)}
                                                {task.assigned && task.assigned.length > 2 && <span className="text-[10px] text-black/40">+{task.assigned.length - 2}</span>}
                                            </div>
                                        </td>
                                        <td className="px-4 py-3">
                                            {subs.length > 0 ? (
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1.5">
                                                <button onClick={() => onEdit(task)} className="rounded-lg border border-black/10 p-1.5 text-black/60 hover:bg-slate-100">
                                                    <Pencil className="h-3.5 w-3.5" />
                                                </button>
                                                <button onClick={() => onDelete(task)} className="rounded-lg border border-rose-200 bg-rose-50 p-1.5 text-rose-600 hover:bg-rose-100">
                                                    <Trash2 className="h-3.5 w-3.5" />
                                                </button>
                                            </div>
                                        </td>
...
function TimelineView({ tasks, onEdit, onDelete, loading }) {
    ...
    const withDate = useMemo(() => tasks.filter(t => (t.start_date && String(t.start_date).trim() !== "") || (t.due_date && String(t.due_date).trim() !== "")), [tasks]);
    const noDate = useMemo(() => tasks.filter(t => !((t.start_date && String(t.start_date).trim() !== "") || (t.due_date && String(t.due_date).trim() !== ""))), [tasks]);
    ...
    const { minDate, totalDays, weeks } = useMemo(() => {
        ...
        const totalDays = Math.max(Math.ceil((max - min) / 86400000), 30);
    ...
    useEffect(() => { 
        if (scrollRef.current && todayOffset > 0 && todayOffset < totalDays) {
    ...
    if (loading) {
        return <div className="rounded-2xl border border-black/10 bg-white p-6 space-y-3">
            {[1,2,3,4].map(i => <div key={i} className="h-20 animate-pulse rounded-xl bg-slate-100" />)}
        </div>;
    }
    
    if (tasks.length === 0) {
        return <div className="rounded-2xl border border-dashed border-black/10 p-10 text-center text-sm text-black/40">No hay planes de acción creados.</div>;
    }
                ...
                <div className="rounded-2xl border border-black/10 bg-white overflow-hidden shadow-sm">
                    <div className="flex items-center justify-between px-4 py-2.5 border-b border-black/[0.07] bg-slate-50">
                        <div className="flex items-center gap-4">
                            ...
                            {[{color:"#2563eb",label:"Hoy"},{color:"#10b981",label:"Hecho"},{color:"#f59e0b",label:"En proceso"},{color:"#94a3b8",label:"Por hacer"},{color:"#ef4444",label:"Vencida"}].map(({color,label}) => (
                    ...
                    <div className="overflow-x-auto" ref={scrollRef} style={{cursor:"grab"}} onMouseDown={(e) => { 
                        ...
                        <div style={{ minWidth: LABEL_W + totalDays * DAY_W, position: "relative" }}>
                            ...
                            <div className="flex border-b border-black/10 sticky top-0 z-30 bg-white">
                                <div className="shrink-0 border-r border-black/10 bg-slate-100 flex items-center justify-center" style={{ width: LABEL_W, minHeight: 36 }}>
                                    <span className="text-[10px] font-extrabold text-black/40 uppercase tracking-widest">Plan de acción</span>
                                </div>
                                {weeks.map((w,i) => (
                                    <div key={i} className="flex items-center justify-center border-r border-black/10 text-[10px] font-bold text-black/40 bg-white" style={{ width: 7 * DAY_W, minHeight: 36, flexShrink: 0 }}>
                                        {w.label}
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="flex border-b border-black/10 sticky top-9 z-30">
                                ...
                                {Array.from({ length: totalDays }).map((_, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className="flex items-center justify-center border-r border-black/[0.06] shrink-0 text-[10px]" style={{ 
                                            width: DAY_W, 
                                            minHeight: 28, 
                            ...
                            <div className="relative">
                                {todayOffset >= 0 && todayOffset < totalDays && (
                                    <div className="absolute top-0 bottom-0 z-20 pointer-events-none" style={{ left: LABEL_W + todayOffset * DAY_W, width: 2, background: "#2563eb" }} />
                                )}
                                ...
                                {Array.from({ length: totalDays }).map((_, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className="absolute top-0 bottom-0 pointer-events-none" style={{ left: LABEL_W + idx * DAY_W, width: DAY_W, background: "rgba(0,0,0,0.018)", zIndex: 0 }} />
                                    );
                                })}
                                
                                {withDate.map((task) => {
                                    let startDate = task.start_date ? new Date(String(task.start_date)) : null;
                                    let dueDate = task.due_date ? new Date(String(task.due_date)) : null;
                                    if (!startDate && dueDate) startDate = new Date(dueDate);
                                    if (!dueDate && startDate) dueDate = new Date(startDate);
                                    ...
                                    return (
                                        <div key={task.id} className="relative border-b border-black/[0.06] group hover:bg-blue-50/20 transition-colors" style={{ minHeight: rowH }}>
                                            {/* Panel izquierdo - información de la tarea */}
                                            <div className="absolute left-0 top-0 border-r border-black/[0.08] bg-white group-hover:bg-blue-50/30 transition-colors z-10 flex flex-col gap-1 px-3 py-2 overflow-hidden" style={{ width: LABEL_W, minHeight: rowH, cursor: "pointer" }} onClick={() => onEdit(task)}>
                                                <div className="flex items-start justify-between gap-1">
                                                    <span className="text-xs font-black text-[#131E5C] leading-snug line-clamp-2 flex-1" title={task.title}>
                                                        {task.title || "Sin título"}
                                                    </span>
                                                    <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                                                        <button onClick={(e) => { e.stopPropagation(); onEdit(task); }} className="p-1 rounded text-black/40 hover:text-[#131E5C] hover:bg-blue-100" title="Editar">
                                                            <Pencil className="h-3 w-3" />
                                                        </button>
                                                        <button onClick={(e) => { e.stopPropagation(); onDelete(task); }} className="p-1 rounded text-rose-400 hover:text-rose-600 hover:bg-rose-50" title="Eliminar">
                                                ...
                                                <div className="flex items-center gap-1 text-[10px] text-black/50">
                                                    ...
                                                    {isOverdue && <span className="text-[9px] font-bold text-red-500 bg-red-50 px-1 rounded">VENCIDA</span>}
                                                </div>
                                                
                                                {task.descripcion_problema && (
                                                    <p className="text-[10px] text-black/50 line-clamp-2 leading-snug">{task.descripcion_problema}</p>
                                                )}
                                                ...
                                                {task.assigned && task.assigned.length > 0 && (
                                                    <div className="flex items-center gap-1 mt-0.5">
                                                        ...
                                                        {task.assigned.slice(0,2).map((a,i) => <UserAvatar key={i} user={a} size="sm" />)}
                                                        {task.assigned.length > 2 && <span className="text-[9px] text-black/40">+{task.assigned.length - 2}</span>}
                                                    </div>
                                                )}
                                                
                                                {/* Subtareas - Sección DESPLEGABLE */}
                                                {subtasks.length > 0 && (
                                                    <div className="mt-0.5">
                                                        <button onClick={(e) => { e.stopPropagation(); toggleRow(task.id); }} className="inline-flex items-center gap-1 text-[10px] font-extrabold text-[#131E5C] hover:underline">
                                                        ...
                                                        {/* Lista de subtareas expandida */}
                                                        {isExpanded && (
                                                            <div className="mt-1.5 space-y-0.5">
                                                                {subtasks.map((sub,i) => (
                                                                    <div key={i} className="flex items-center gap-1.5 text-[10px] text-black/60">
                                                                        {sub.done ? <CheckCircle2 className="h-3 w-3 text-emerald-500 shrink-0" /> : <Clock3 className="h-3 w-3 text-slate-400 shrink-0" />}
                                            ...
                                            <div className="absolute z-10 rounded-lg cursor-pointer transition-all hover:opacity-90 hover:-translate-y-0.5" style={{ 
                                                left: LABEL_W + startOff * DAY_W + 3, 
                                                width: Math.max(dur * DAY_W - 6, 60), 
                                                top: 10, 
                                                height: Math.min(40, rowH - 20), 
                                                background: bc + "28", 
                                            ...
                                            }} onClick={() => onEdit(task)} title={`${task.title}\n${fmtDate(startDate)} → ${fmtDate(dueDate)}`}>
                                                <div className="flex flex-col justify-center h-full px-2 overflow-hidden">
                                                    <span className="text-[11px] font-bold truncate leading-tight" style={{ color: bc }}>{task.title}</span>
                                                    <span className="text-[9px] text-black/50 truncate">{fmtDate(startDate)} → {fmtDate(dueDate)}</span>
                                                </div>
                                                {progress > 0 && (
                                                    <div className="absolute bottom-0 left-0 right-0 h-1 rounded-b-lg overflow-hidden" style={{ background: bc + "20" }}>
                                                        <div className="h-full rounded-b-lg" style={{ width: `${progress}%`, background: bc }} />
                                                    </div>
                                                )}
                                            </div>
                                            
                                            {/* Mini-barras de subtareas en la línea de tiempo (cuando están expandidas) */}
                                            {isExpanded && subtasks.map((sub, i) => {
                                                ...
                                                const subDur = i === totalSubs - 1 ? dur - i * subDurDays : subDurDays;
                                                const subColor = sub.done ? "#10b981" : "#94a3b8";
                                                const topPos = ROW_BASE + i * SUB_H - 2;
                                                
                                                return (
                                                    <div key={i} className="absolute z-10 rounded flex items-center px-2" style={{ 
            ...
            {noDate.length > 0 && (
                <div className="rounded-2xl border border-dashed border-amber-200 bg-amber-50/30 p-4">
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs font-extrabold text-amber-700 uppercase tracking-wide">Sin fecha asignada ({noDate.length})</span>
                    </div>
                    <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                        {noDate.map(task => (
                            <div key={task.id} className="flex items-center justify-between gap-2 rounded-xl border border-amber-200 bg-white p-3 hover:bg-amber-50 cursor-pointer transition" onClick={() => onEdit(task)}>
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                                        <span className="text-sm font-black text-[#131E5C] truncate">{task.title || "Sin título"}</span>
                                        <PriorityBadge value={task.priority} />
                                    </div>
                                    <StatusBadge name={task.list_name} />
                                    {task.descripcion_problema && (
                                        <p className="text-[10px] text-black/50 line-clamp-1 mt-1">{task.descripcion_problema}</p>
                                    )}
                                    {task.desarrollo_estrategia && (
                                        <p className="text-[9px] text-blue-600 line-clamp-1 mt-0.5">📋 {task.desarrollo_estrategia.slice(0, 40)}</p>
                                    )}
                                    {task.assigned && task.assigned.length > 0 && (
                                ...
                                <button onClick={(e) => { e.stopPropagation(); onDelete(task); }} className="shrink-0 rounded-lg border border-rose-200 bg-rose-50 p-1.5 text-rose-600 hover:bg-rose-100">
                                    <Trash2 className="h-3.5 w-3.5" />
                                </button>
                            </div>
                        ))}
                    </div>
                    <p className="mt-3 text-xs text-amber-600 text-center">Asigna una fecha de inicio o límite para ver tareas en la línea de tiempo.</p>
                </div>
            )}
        </div>
    );
...
export default function TimeForAction() {
    ...
    const getCurrentUser = useCallback(() => {
        try {
            ...
            if (auth) {
                ...
                const userData = parsed?.user || parsed?.usuario;
                if (userData) {
                    setCurrentUser({
    ...
    const loadBoard = useCallback(async () => {
        ...
        try {
            ...
            setLists(rawLists);
            const flat = rawLists.flatMap(l => (tasksByList[l.id] || []).map(t => ({ ...t, list_name: l.name, list_id: l.id })));
            setTasks(flat);
        } catch (e) { console.error(e); }
        finally { setLoading(false); }
    }, [teamId, projectId]);
    ...
    const filtered = useMemo(() => {
        ...
        return tasks.filter(t => {
            ...
            if (showMyTasksOnly && currentUser) {
                matchMyTasks = t.assigned && t.assigned.some(a => a.user_id === currentUser.id || a.id === currentUser.id);
            }
            return matchQ && matchS && matchMyTasks;
        });
    ...
    const statCounts = useMemo(() => {
        ...
        if (showMyTasksOnly && currentUser) {
            const myTasks = tasks.filter(t => t.assigned && t.assigned.some(a => a.user_id === currentUser.id || a.id === currentUser.id));
            out = {};
            for (const col of STATUS_COLS) out[col] = myTasks.filter(t => t.list_name === col).length;
        }
        return out;
    }, [tasks, showMyTasksOnly, currentUser]);
    ...
    return (
        <div className="w-full space-y-4">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex items-center gap-2">
                    <button onClick={() => setTeamsModalOpen(true)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#131E5C] bg-white px-4 py-2 text-sm font-extrabold text-[#131E5C] hover:bg-slate-50">
                        <UsersRound className="h-4 w-4" />Equipos
                    </button>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {editingProject ? (
                        ...
                        <div className="flex items-center gap-1.5">
                            ...
                            {projectId && (
                                <button onClick={() => setConfirmDeleteProject(true)} className="rounded-xl border border-rose-200 bg-rose-50 p-1.5 text-rose-600 hover:bg-rose-100" title="Eliminar proyecto"><Trash2 className="h-3.5 w-3.5" /></button>
                            )}
                        </div>
                    )}
                    <button onClick={() => setProjectModalOpen(true)} className="inline-flex items-center gap-1.5 rounded-xl border border-[#131E5C] bg-white px-3 py-1.5 text-xs font-extrabold text-[#131E5C] hover:bg-slate-50">
            ...
            <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                    ...
                    className={cls(
                        ...
                        showMyTasksOnly ? "bg-[#131E5C] text-white" : "border border-[#131E5C] text-[#131E5C] hover:bg-slate-50"
                    )}
                >
                    <UsersRound className="h-3.5 w-3.5" />
                    Mis tareas
            ...
            <ConfirmDialog open={confirmDeleteProject} title="Eliminar proyecto" message={`¿Seguro que deseas eliminar "${currentProject?.name}"? Se eliminarán todos sus planes. Esta acción no se puede deshacer.`} onConfirm={deleteCurrentProject} onCancel={() => setConfirmDeleteProject(false)} loading={deletingProject} />
